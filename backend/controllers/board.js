...
const saveTaskImg = async (req, res) => {
  ...
  if (req.files !== undefined && req.files.image.type) {
    let url = req.protocol + "://" + req.get("host") + "/"; //con el req.protocol esto esta sacando el protocolo con el que llego el cual seria http
    let serverImg =
      "./uploads/" + moment().unix() + path.extname(req.files.image.path); //con esto sacamos la ext de la imagen para saber si es jpg o png, etc.
    fs.createReadStream(req.files.image.path).pipe(
      fs.createWriteStream(serverImg)
    ); //con esto nuestra imagen queda guardada en el proyecto en la carpeta uploads
    imageUrl =
...
const deleteTask = async (req, res) => {
  ...
  if (!board) return res.status(400).send("Task not found");
  return res.status(200).send({message: "Task deleted"}); //con el {message: } convertimos ese string a json para que asi se pueda comunicar satisfactoriamente con el frontend
};
